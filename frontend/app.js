 ...
 * Copyright (c) 2025 Fundacja na rzecz Hospicjum Maryi KrÛlowej Aposto≥Ûw w Krakowie
 */
...
window.testSpeech = function() {
    console.log('?? TEST: KlikniÍto przycisk test mowy');
    const testMessage = "Witaj! To jest test mowy. Jeúli mnie s≥yszysz, znaczy øe wszystko dzia≥a!";
    ...
    if (window.speechSynthesis) {
        ...
        utterance.onstart = () => console.log('?? Test mowy rozpoczÍty');
        utterance.onend = () => console.log('? Test mowy zakoÒczony');
        utterance.onerror = (e) => console.log('? B≥πd test mowy:', e.error);
        
        window.speechSynthesis.speak(utterance);
        console.log('?? Test speak() wys≥any');
    } else {
        console.log('? Brak TTS w przeglπdarce');
        alert('Twoja przeglπdarka nie obs≥uguje mowy!');
    }
};
...
function showLoader(targetElement, message = '£adujÍ...') {
...
function showMascotLoader(message = '?? ChwileczkÍ...') {
...
function addLearnedTip(tipTitle) {
    if (!userMemory.learnedTips.includes(tipTitle)) {
        ...
        if (userMemory.learnedTips.length === 5 && !userMemory.achievements.includes('safety_expert')) {
            ...
            showAchievement('?? Ekspert BezpieczeÒstwa!', 'Pozna≥eú wszystkie 5 porad bezpieczeÒstwa!');
        }
...
function showAchievement(title, description) {
    ...
    if (speechEnabled) {
        speakText(`OsiπgniÍcie odblokowane! ${title}. ${description}`);
    }
}
...
function initMap() {
    ...
    const mapLoader = showLoader(mapElement, '??? £adujÍ mapÍ Polski...');
 ...
 * 112 õ "jeden jeden dwa", 997 õ "dziewiÍÊ dziewiÍÊ siedem"
 */
function formatNumbersForChildren(text) {
    ...
    // Standalone numbers after "dzwoÒ" or "wybierz"
    text = text.replace(/\b(dzwoÒ|wybierz|naciúnij)\s+(\d{3,})\b/gi, (match, action, number) => {
...
function formatTextForCzubowna(text) {
    ...
    // After important words (bezpieczeÒstwo, lokalizacja, etc.)
    formatted = formatted.replace(/\b(bezpieczeÒstwo|lokalizacja|dziecko|rodzic|pomoc|zagroøenie)\b/gi, '$1...');
    
    // After conjunctions (natural speech breaks)
    formatted = formatted.replace(/\b(ale|wiÍc|czyli|oraz|a takøe|i)\b/gi, '$1...');
    
    // Before important information
    formatted = formatted.replace(/\b(uwaga|pamiÍtaj|waøne|ostrzeøenie)\b/gi, '...$1');
...
function selectCzubownaLikeVoice(lang = 'pl') {
    ...
    const polishFemale = voices.find(v => 
        ...
        !v.name.toLowerCase().includes('mÍøczyzna')
    );
...
function toggleSpeech() {
    ...
    if (btn) {
        btn.innerHTML = speechEnabled ? 
            '<span class="btn-icon">??</span><span class="btn-text">Czytanie w≥πczone</span>' :
            '<span class="btn-icon">??</span><span class="btn-text">Czytanie wy≥πczone</span>';
...
function generateLocationMessage() {
    ...
    if (!hasLLM) {
        ...
        return {
            text: '?? Sprawdzam twojπ lokalizacjÍ...',
            speech: 'Sprawdzam twojπ lokalizacjÍ'
        };
    }
    ...
    if (childAge <= 6) {
        ...
        baseMessage = '?? Szukam gdzie jesteú, øeby ci pomÛc!';
        speechMessage = 'Szukam... gdzie... jesteú... øeby... ci... pomÛc';
    } else if (childAge <= 9) {
        // Young children (7-9) 
        baseMessage = '?? Sprawdzam twojπ lokalizacjÍ, øebyú by≥ bezpieczny!';
        speechMessage = 'Sprawdzam... twojπ... lokalizacjÍ... øebyú... by≥... bezpieczny';
    } else if (childAge <= 12) {
        // Pre-teens (10-12)
        baseMessage = '?? Okreúlam twojπ pozycjÍ dla twojego bezpieczeÒstwa!';
        speechMessage = 'Okreúlam... twojπ... pozycjÍ... dla... twojego... bezpieczeÒstwa';
    } else {
        // Teenagers (13-16)
        baseMessage = '?? AnalizujÍ twojπ lokalizacjÍ w systemie bezpieczeÒstwa!';
        speechMessage = 'AnalizujÍ... twojπ... lokalizacjÍ... w... systemie... bezpieczeÒstwa';
    }
...
async function enhanceLocationMessageWithAI(childAge, fallbackMessage) {
    try {
        const prompt = `Jako przyjazny asystent dla ${childAge}-letniego dziecka, napisz krÛtkπ (max 50 znakÛw), pozytywnπ wiadomoúÊ o sprawdzaniu lokalizacji dziecka dla bezpieczeÒstwa. Uøyj prostego jÍzyka i emotikonki. Przyk≥ad: "?? Szukam gdzie jesteú!"`;
...
function getUserLocation(userRequested = false) {
    ...
    if (!navigator.geolocation) {
        ...
        handleLocationError('Twoja przeglπdarka nie obs≥uguje lokalizacji.');
        return;
    }
...
function handleLocationError(error) {
    ...
    const errorType = typeof error === 'string' ? error : error.message || 'Nieznany b≥πd';
    
    if (childAge <= 6) {
        message = '?? Nie mogÍ znaleüÊ gdzie jesteú, ale nie martw siÍ! Zapytaj doros≥ego o pomoc.';
        speech = 'Nie... mogÍ... znaleüÊ... gdzie... jesteú... ale... nie... martw... siÍ... Zapytaj... doros≥ego... o... pomoc';
        advice = 'Poproú mamÍ lub tatÍ øeby w≥πczyli lokalizacjÍ w telefonie.';
    } else if (childAge <= 9) {
        message = '?? Mam problem ze znalezieniem Twojej lokalizacji. To moøe byÊ przez ustawienia telefonu.';
        speech = 'Mam... problem... ze... znalezieniem... Twojej... lokalizacji... To... moøe... byÊ... przez... ustawienia... telefonu';
        advice = 'Sprawdü czy w≥πczy≥eú lokalizacjÍ w ustawieniach lub zapytaj doros≥ego.';
    } else if (childAge <= 12) {
        message = '?? Nie uda≥o siÍ ustaliÊ lokalizacji. Moøesz uøyÊ aplikacji bez tego, ale niektÛre funkcje bÍdπ ograniczone.';
        speech = 'Nie... uda≥o... siÍ... ustaliÊ... lokalizacji... Moøesz... uøyÊ... aplikacji... bez... tego';
        advice = 'Sprawdü ustawienia lokalizacji w przeglπdarce lub sprÛbuj ponownie.';
    } else {
        message = '?? B≥πd geolokalizacji: Brak dostÍpu do GPS. Aplikacja bÍdzie dzia≥aÊ w trybie podstawowym.';
        speech = 'B≥πd... geolokalizacji... Brak... dostÍpu... do... GPS... Aplikacja... bÍdzie... dzia≥aÊ... w... trybie... podstawowym';
        advice = 'W≥πcz lokalizacjÍ w ustawieniach przeglπdarki lub urzπdzenia.';
    }
...
function makeAddressChildFriendly(fullAddress) {
    ...
    for (let part of parts) {
        ...
        if (part.toLowerCase().includes('podgÛrze') || 
            part.toLowerCase().includes('krowodrza') ||
            part.toLowerCase().includes('úrÛdmieúcie') ||
        ...
        if (part.toLowerCase().includes('osiedle') || 
            part.toLowerCase().includes('kurdwanÛw') ||
            part.toLowerCase().includes('prπdnik') ||
            part.toLowerCase().includes('bronowice')) {
            neighborhood = part;
        }
        
        // Look for city
        if (part.toLowerCase().includes('krakÛw') ||
            part.toLowerCase().includes('warszawa') ||
            part.toLowerCase().includes('gdaÒsk') ||
            part.toLowerCase().includes('wroc≥aw') ||
            part.toLowerCase().includes('poznaÒ')) {
            city = part;
        }
    }
    ...
    if (district && neighborhood) {
        ...
        // Fallback: just city
        friendlyAddress.push(`mieúcie ${city}`);
    }
 ...
 * Bezpoúrednie pobieranie z RSS RCB gdy backend niedostÍpny
 */
                ...
                for (let item of (data.items || [])) {
                    ...
                    if (hoursDiff <= 48) {
                        alerts.push({
                            ...
                            content: item.description || item.content || 'Sprawdü szczegÛ≥y na stronie RCB',
                            severity: 'high',
        ...
        },
        {
            ...
            fetch: async () => {
                ...
                for (let item of items) {
                    ...
                    const description = item.querySelector('description')?.textContent || 'Sprawdü szczegÛ≥y';
 ...
 * Priority: GPS+ParentRegion õ GPS õ National õ Always Critical
 */
...
async function generateEmergencyFallbackMessage(alert, childAge) {
    try {
        ...
        const safeDefaults = {
            young: `?? Uwaga, kochanie! Jest waøne ostrzeøenie w okolicy. Znajdü mamÍ, tatÍ lub innego doros≥ego i zostaÒ przy nim. Doroúli wiedzπ co robiÊ i zadbajπ o Twoje bezpieczeÒstwo.`,
            older: `?? Uwaga! Otrzymano waøne ostrzeøenie bezpieczeÒstwa. Skontaktuj siÍ z rodzicami lub znajdü doros≥ego. S≥uchaj instrukcji doros≥ych i s≥uøb ratunkowych.`
        };
        ...
        // Ultra-safe fallback
        return `?? Waøne ostrzeøenie! Znajdü doros≥ego i zostaÒ przy nim. Doroúli zadbajπ o Twoje bezpieczeÒstwo.`;
    }
}
...
async function getParentCustomMessage(alert, childAge) {
    try {
        ...
        if (title.includes('woda niezdatna') || content.includes('nie nadaje siÍ do spoøycia')) {
            alertType = 'water';
        } else if (title.includes('burza') || title.includes('wiatr') || title.includes('grad')) {
            alertType = 'storm';
        } else if (title.includes('powÛdü') || content.includes('podtopienia')) {
            alertType = 'flood';
        } else if (title.includes('dron') || title.includes('obiekt')) {
            alertType = 'drones';
        } else if (title.includes('Êwiczenia')) {
            alertType = 'exercises';
        }
...
async function generateAlertMessage(alert, childAge) {
    ...
    const prompt = `Jako asystent bezpieczeÒstwa dla ${childAge}-letniego dziecka, przet≥umacz ten oficjalny alert na prosty, uspokajajπcy jÍzyk:
...
Wytyczne:
- Uøyj prostego jÍzyka dla ${childAge}-latka
- Zacznij od odpowiedniego emoji
- Maksymalnie 2-3 zdania
- Podaj konkretne, bezpieczne dzia≥ania
- Nie strasü, ale ostrzegaj
- Podkreúl "poproú doros≥ego o pomoc"
...
function generateRuleBasedAlert(alert, childAge) {
    ...
    if (title.includes('burza') || title.includes('wiatr') || title.includes('grad') || content.includes('burze')) {
        emoji = '??';
        if (childAge <= 6) {
            message = 'Nadchodzi burza z gradem! Idü szybko do domu. Nie baw siÍ na dworze. Schowaj zabawki do úrodka.';
        } else if (childAge <= 9) {
            message = 'Alert burzowy! WrÛÊ natychmiast do domu. Unikaj drzew i wysokich budynkÛw. Zamknij okna.';
        } else {
            message = 'Ostrzeøenie przed burzami i gradem. Szukaj natychmiastowego schronienia. Zabezpiecz rzeczy na balkonie.';
        }
    } else if (title.includes('woda niezdatna') || content.includes('nie nadaje siÍ do spoøycia') || content.includes('nie nadaje siÍ do uøytku')) {
        emoji = '??';
        if (childAge <= 6) {
            message = 'Nie pij wody z kranu! Woda jest brudna i moøe ciÍ rozchorowaÊ. Pij tylko wodÍ z butelek.';
        } else if (childAge <= 9) {
            message = 'Woda z kranu jest skaøona! Uøywaj tylko wody butelkowanej do picia, mycia zÍbÛw i gotowania.';
        } else {
            message = 'Skaøenie wody pitnej! Nie uøywaj wody z kranu do picia ani przygotowania jedzenia. Tylko woda butelkowana!';
        }
    } else if (title.includes('powÛdü') || content.includes('podtopienia') || content.includes('wezbranych') || content.includes('opady deszczu')) {
        emoji = '??';
        if (childAge <= 6) {
            message = 'Za duøo wody wszÍdzie! Nie chodü blisko rzek ani potoczkÛw. Trzymaj siÍ z dala od ka≥uø.';
        } else if (childAge <= 9) {
            message = 'Niebezpieczne podtopienia! Unikaj rzek, mostÛw i niskich terenÛw. Idü na wyøsze miejsce.';
        } else {
            message = 'Alert powodziowy! Natychmiast oddal siÍ od rzek i potokÛw. Udaj siÍ na wyøszy teren.';
        }
    } else if (title.includes('dron') || title.includes('obiekt') || content.includes('naruszy≥y granice')) {
        emoji = '??';
        if (childAge <= 6) {
            message = 'Niebezpieczne latajπce rzeczy! Idü szybko do domu. Nie dotykaj niczego co spad≥o z nieba.';
        } else if (childAge <= 9) {
            message = 'Niebezpieczne drony w okolicy! WrÛÊ do domu. Jeúli widzisz coú spadajπcego - nie zbliøaj siÍ!';
        } else {
            message = 'Alert wojskowy - drony! Natychmiastowe schronienie. Nie dotykaj podejrzanych obiektÛw!';
        }
    } else if (title.includes('Êwiczenia') || content.includes('strza≥Ûw') || content.includes('helikopterÛw')) {
        emoji = '??';
        if (childAge <= 6) {
            message = 'Wojsko Êwiczy dzisiaj. BÍdzie g≥oúno ale to nie prawdziwa wojna. ZostaÒ blisko doros≥ych.';
        } else if (childAge <= 9) {
            message = '∆wiczenia wojskowe w okolicy. Odg≥osy strza≥Ûw i helikopterÛw to tylko trening. Wszystko w porzπdku.';
        } else {
            message = '∆wiczenia s≥uøb mundurowych. Ha≥as strza≥Ûw i loty helikopterÛw to czÍúÊ treningu, zachowaj spokÛj.';
        }
    } else {
        // Generic alert with concrete actions
        if (childAge <= 6) {
            message = 'Waøne ostrzeøenie! Znajdü szybko doros≥ego i powiedz mu o tej wiadomoúci.';
        } else if (childAge <= 9) {
            message = 'Alert bezpieczeÒstwa! Poinformuj rodzicÛw i trzymaj siÍ blisko domu.';
        } else {
            message = 'Ostrzeøenie w twojej okolicy. Sprawdü z rodzicami co robiÊ dalej.';
        }
    }
    
    return `${emoji} ${message} Zawsze poproú doros≥ego o pomoc!`;
}
...
function testAlert(alertType = 'weather') {
    ...
    const testAlerts = {
        water: {
            ...
            content: 'UWAGA! Woda z wodociπgu w gminie nie nadaje siÍ do spoøycia. Gmina zapewnia wodÍ pitnπ w butelkach i z beczkowozu. åledü lokalne komunikaty.',
            severity: 'critical',
        ...
        storm: {
            ...
            content: 'Uwaga! Dziú moøliwe burze, silny wiatr i lokalnie grad. Zachowaj ostroønoúÊ i zabezpiecz rzeczy, ktÛre moøe porwaÊ wiatr.',
            severity: 'warning',
            location: 'WojewÛdztwo Ma≥opolskie',
            timestamp: new Date().toISOString()
        },
        flood: {
            id: 'rcb_flood_real',
            title: 'Alert RCB - intensywne opady deszczu i burze',
            content: 'Uwaga! Dziú prognozowane intensywne opady deszczu i burze. Moøliwe podtopienia. Nie zbliøaj siÍ do wezbranych rzek. S≥uchaj poleceÒ s≥uøb.',
            severity: 'warning',
        ...
        drones: {
            id: 'rcb_drones_real',
            title: 'Alert RCB - neutralizacja obiektÛw',
            content: 'Uwaga! W zwiπzku z operacjπ neutralizacji obiektÛw, ktÛre naruszy≥y granice RP, informuj s≥uøby o dronach lub miejscach ich upadku. Nie zbliøaj siÍ do nich.',
            severity: 'critical',
            location: 'WojewÛdztwa wschodnie',
            timestamp: new Date().toISOString()
        },
        exercises: {
            id: 'rcb_exercises_real', 
            title: 'Alert RCB - Êwiczenia s≥uøb mundurowych',
            content: 'Uwaga! ∆wiczenia s≥uøb mundurowych. Moøliwe odg≥osy strza≥Ûw i loty helikopterÛw. Zachowaj spokÛj i ostroønoúÊ.',
            severity: 'info',
            location: 'Teren ÊwiczeÒ',
            timestamp: new Date().toISOString()
        }
    };
...
function addTestButtons() {
    ...
    testContainer.innerHTML = `
        <div style="font-weight: bold; margin-bottom: 10px;">?? Test Rzeczywistych AlertÛw RCB</div>
        ...
        <button onclick="testAlert('flood')" style="margin: 5px; padding: 8px 12px; border: none; border-radius: 5px; background: #2196F3; color: white; cursor: pointer;">?? PowÛdü</button>
    ...
    switch(action) {
        case 'find_safety':
            if (complexity === 'simple') {
                return `?? ${agePrefix ? 'Maluszku, szukaj' : 'Szukaj'} bezpiecznych miejsc! Idü do sklepu, szko≥y lub tam gdzie sπ doroúli ludzie!`;
            } else if (complexity === 'medium') {
                return `?? Gdy siÍ zgubisz, szukaj bezpiecznych miejsc: sklepy, szko≥y, biblioteki. Zawsze tam gdzie sπ doroúli!`;
            } else {
                return `?? Najbliøsze bezpieczne miejsca to: sklepy, szko≥y, biblioteki, komisariaty i urzÍdy. Wybieraj miejsca z dobrym oúwietleniem i wieloma osobami.`;
            }
            
        case 'safe_route':
            if (complexity === 'simple') {
                return `?? ${agePrefix ? 'Maluszku, i' : 'I'}dü g≥Ûwnπ drogπ! Nie skrÍcaj w ciemne uliczki. Przechodü tylko tam gdzie sπ pasy!`;
            } else if (complexity === 'medium') {
                return `?? Bezpieczna droga: idü g≥Ûwnymi ulicami, gdzie jest duøo ludzi. Unikaj pustych miejsc!`;
            } else {
                return `?? Planuj bezpiecznπ trasÍ: g≥Ûwne ulice, dobrze oúwietlone miejsca, przejúcia dla pieszych. Unikaj skrÛtÛw przez parki czy pustynie tereny.`;
            }
            
        case 'emergency_help':
            if (complexity === 'simple') {
                return `?? ${agePrefix ? 'Maluszku, w' : 'W'} niebezpieczeÒstwie dzwoÒ 112! Poproú doros≥ego o pomoc!`;
            } else if (complexity === 'medium') {
                return `?? W sytuacji awaryjnej: dzwoÒ 112, znajdü doros≥ego, idü do bezpiecznego miejsca!`;
            } else {
                return `?? Procedura awaryjna: 1) OceÒ sytuacjÍ 2) DzwoÒ 112 3) Poinformuj zaufanego doros≥ego 4) Idü do najbliøszego bezpiecznego miejsca`;
            }
        ...
        case 'welcome':
            if (isFirstVisit) {
                if (complexity === 'simple') {
                    return `?? CzeúÊ ${agePrefix}! Jestem twoim przyjacielem, ktÛry pomoøe ci byÊ bezpiecznym!`;
                } else if (complexity === 'medium') {
                    return `?? Witaj! Jestem twoim pomocnikiem bezpieczeÒstwa. PokaøÍ ci jak byÊ bezpiecznym!`;
                } else {
                    return `??? CzeúÊ! Jestem AI asystentem bezpieczeÒstwa. PomogÍ ci w rÛønych sytuacjach!`;
                }
            } else {
                if (complexity === 'simple') {
                    return `?? CzeúÊ ${agePrefix}! Mi≥o ciÍ znowu widzieÊ!`;
                } else if (complexity === 'medium') {
                    return `?? Witaj ponownie! Super, øe wrÛci≥eú!`;
                } else {
                    return `??? Witaj z powrotem! Gotowy na nowe przygody z bezpieczeÒstwem?`;
                }
            }
            
        default:
            return 'CzeúÊ! Jestem twoim pomocnikiem bezpieczeÒstwa!';
    }
}
...
async function handleWhereAmI() {
    ...
    showMascotLoader('?? Szukam gdzie jesteú...');
...
async function handleFindSafety() {
    ...
    const mascotText = document.getElementById('mascot-text');
    if (mascotText) {
        mascotText.textContent = '?? Szukam bezpiecznych miejsc w pobliøu...';
    }
    ...
    if (navigator.geolocation) {
        try {
            ...
            if (mascotText) {
                mascotText.textContent = '?? Nie uda≥o siÍ okreúliÊ lokalizacji. Sprawdü uprawnienia GPS.';
            }
        }
    }
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.querySelectorAll('.emergency-card').forEach(card => {
        card.addEventListener('click', () => {
            ...
            if (h4 && link) {
                ...
                const mascotText = document.getElementById('mascot-text');
                if (mascotText) {
                    mascotText.textContent = `?? ${service} - numer ${number}. DzwoÒ tylko w prawdziwych sytuacjach awaryjnych!`;
                }
                
                speakText(`${service}. Numer ${number}. DzwoÒ tylko w prawdziwych sytuacjach awaryjnych!`);
        ...
        notificationsBtn.addEventListener('click', () => {
            ...
            if ('Notification' in window) {
                if (Notification.permission === 'granted') {
                    new Notification('Bezpieczny Pomocnik', {
                        body: 'Powiadomienia sπ juø w≥πczone!',
                        icon: 'images/logo_192x192.png'
                    });
                    
                    const mascotText = document.getElementById('mascot-text');
                    if (mascotText) {
                        mascotText.textContent = '?? Powiadomienia sπ juø w≥πczone!';
                    }
                    
                    speakText('Powiadomienia sπ juø w≥πczone');
                } else if (Notification.permission !== 'denied') {
                    Notification.requestPermission().then(permission => {
                        if (permission === 'granted') {
                            new Notification('Bezpieczny Pomocnik', {
                                body: 'åwietnie! Powiadomienia zosta≥y w≥πczone!',
                                icon: 'images/logo_192x192.png'
                            });
                            
                            notificationsBtn.innerHTML = '<span class="btn-icon">?</span><span class="btn-text">Powiadomienia w≥πczone</span>';
                            notificationsBtn.style.background = '#32D74B';
                            
                            const mascotText = document.getElementById('mascot-text');
                            if (mascotText) {
                                mascotText.textContent = '? åwietnie! Powiadomienia zosta≥y w≥πczone!';
                            }
                            
                            speakText('åwietnie! Powiadomienia zosta≥y w≥πczone');
                        }
                    });
                }
            } else {
                const mascotText = document.getElementById('mascot-text');
                if (mascotText) {
                    mascotText.textContent = '? Twoja przeglπdarka nie obs≥uguje powiadomieÒ';
                }
                
                speakText('Twoja przeglπdarka nie obs≥uguje powiadomieÒ');
            }
        });
    }
    ...
    function showCallPopup(phoneNumber, targetElement) {
        ...
        popup.innerHTML = `
            <div class="popup-content">
                <h3>?? Po≥πczenie</h3>
                <p>Czy chcesz zadzwoniÊ pod numer:</p>
                <div class="phone-display">${phoneNumber}</div>
                <p><small>Kliknij ponownie aby wykonaÊ po≥πczenie</small></p>
    ...
    setTimeout(() => {
        ...
        if (mascotText && userMemory.visitCount === 1) {
            mascotText.textContent = 'CzeúÊ! Jestem twoim pomocnikiem bezpieczeÒstwa!';
        }
    }, 2000);
    ...
    try {
        ...
        if (window.getParentLocation) {
            ...
            if (parentLocation && parentLocation.lat && parentLocation.lon) {
                ...
                const mascotText = document.getElementById('mascot-text');
                if (mascotText) {
                    mascotText.textContent = '? Znalaz≥em rodzicÛw! Zobacz ich lokalizacjÍ na mapie.';
                }
                
                speakText('Znalaz≥em rodzicÛw! Zobacz ich lokalizacjÍ na mapie.');
                return;
            }
        }
        ...
        if (window.getHomeLocation) {
            ...
            if (homeLocation && homeLocation.lat && homeLocation.lon) {
                ...
                const mascotText = document.getElementById('mascot-text');
                if (mascotText) {
                    mascotText.textContent = '?? Nie znalaz≥em aktualnej lokalizacji rodzicÛw, ale pokazujÍ Wam adres domowy. Rodzice na pewno sπ w pobliøu i wszystko jest w porzπdku!';
                }
                
                speakText('Nie znalaz≥em aktualnej lokalizacji rodzicÛw, ale pokazujÍ wam adres domowy. Rodzice na pewno sπ w pobliøu i wszystko jest w porzπdku!');
                return;
            }
        }
        ...
        const mascotText = document.getElementById('mascot-text');
        if (mascotText) {
            mascotText.textContent = '?? Nie mogÍ teraz pokazaÊ lokalizacji rodzicÛw, ale to nie znaczy, øe coú z≥ego siÍ dzieje! Rodzice czÍsto nie udostÍpniajπ lokalizacji, gdy sπ w bezpiecznym miejscu. Jeúli martwisz siÍ, moøesz do nich zadzwoniÊ - na pewno odpowiedzπ! ¢?';
        }
        
        speakText('Nie mogÍ teraz pokazaÊ lokalizacji rodzicÛw, ale to nie znaczy, øe coú z≥ego siÍ dzieje! Rodzice czÍsto nie udostÍpniajπ lokalizacji, gdy sπ w bezpiecznym miejscu. Jeúli martwisz siÍ, moøesz do nich zadzwoniÊ - na pewno odpowiedzπ!');
        ...
        const mascotText = document.getElementById('mascot-text');
        if (mascotText) {
            mascotText.textContent = '?? Nie mogÍ teraz sprawdziÊ lokalizacji rodzicÛw, ale to nic z≥ego! Jeúli chcesz z nimi porozmawiaÊ, moøesz do nich zadzwoniÊ. Na pewno wszystko jest w porzπdku!';
        }
        
        speakText('Nie mogÍ teraz sprawdziÊ lokalizacji rodzicÛw, ale to nic z≥ego! Jeúli chcesz z nimi porozmawiaÊ, moøesz do nich zadzwoniÊ. Na pewno wszystko jest w porzπdku!');
    }
}
...
function showAlertMonitoringStatus() {
    ...
    statusDiv.innerHTML = '?? System AlertÛw: AKTYWNY';
...
function showDetailedAlertStatus() {
    ...
    content.innerHTML = `
        <h3>?? Status Systemu AlertÛw</h3>
        <p><strong>?? Monitoring:</strong> ${alertMonitoringActive ? (alertSourceWorking ? 'AKTYWNY ?' : 'OFFLINE - brak ürÛd≥a ??') : 'WY£•CZONY ?'}</p>
        <p><strong>?? èrÛd≥o alertÛw:</strong> ${alertSourceWorking ? `RCB RSS (ostatnie: ${lastSuccessfulFetch ? new Date(lastSuccessfulFetch).toLocaleTimeString('pl-PL') : 'nigdy'})` : 'NiedostÍpne'}</p>
        <p><strong>? CzÍstotliwoúÊ:</strong> Co 90 sekund (tryb deweloperski)</p>
        <p><strong>?? Lokalizacja:</strong> ${userLocation ? 'W≥πczona' : 'Wy≥πczona'}</p>
        <p><strong>?? Aktywne alerty:</strong> ${activeAlerts.length}</p>
        <p><strong>?? èrÛd≥a danych:</strong></p>
        <ul>
        ...
        <p style="color: #4CAF50;">? <strong>Background alerts</strong> - dzia≥a gdy app zamkniÍty</p>
    ...
    tips.forEach((tip, index) => {
        ...
        if (tip.title || tip.content) {
            ...
            tipCard.innerHTML = `
                ...
                <h4>${tip.title || 'Zasada bezpieczeÒstwa'}</h4>
                <p>${tip.content || ''}</p>
            `;
