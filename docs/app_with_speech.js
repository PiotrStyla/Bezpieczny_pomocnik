document.addEventListener('DOMContentLoaded', () => {
    ...
    const mascotMessages = {
        pl: {
            ...
            speechEnabled: "Teraz będę czytać ci wiadomości na głos! 🔊",
            speechDisabled: "Nie będę już czytać na głos. 🔇"
        },
        en: {
            ...
            speechEnabled: "Now I will read messages aloud to you! 🔊", 
            speechDisabled: "I won't read aloud anymore. 🔇"
        },
        ua: {
            ...
            speechEnabled: "Тепер я читатиму повідомлення вголос! 🔊",
            speechDisabled: "Я більше не читатиму вголос. 🔇"
        }
    };
    ...
    function speakText(text, lang = 'pl') {
        ...
        // Try to find a female voice (more child-friendly)
        const voices = speechSynthesis.getVoices();
        const preferredVoice = voices.find(voice => 
            voice.lang.startsWith(langMap[lang]) && 
            voice.name.toLowerCase().includes('female')
        ) || voices.find(voice => voice.lang.startsWith(langMap[lang]));
    ...
    function toggleSpeech() {
        ...
        if (speechEnabled) {
            ...
            updateMascotMessage('speechEnabled');
            speakText("Czytanie na głos jest teraz włączone!", currentLang);
        } else {
            ...
            speechSynthesis.cancel();
            updateMascotMessage('speechDisabled');
        }
    }
    ...
    function updateMascotMessage(messageKey, ...args) {
        ...
        if (mascotText) {
            ...
            // Read mascot message aloud if speech is enabled
            if (speechEnabled && messageKey !== 'speechEnabled' && messageKey !== 'speechDisabled') {
                setTimeout(() => speakText(message, currentLang), 500);
            }
    ...
    function renderAlert(alert) {
        ...
        // Add speech button for each alert
        const speechButtonHtml = `
            <button class="speech-btn" onclick="speakAlert('${alert.id}')" title="Posłuchaj tego alertu">
                🔊 Posłuchaj
            </button>
        `;
        ...
        card.innerHTML = `
            <div class="alert-content">
                ...
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 12px;">
                    <h2 style="flex: 1; margin: 0;">${alert.title}</h2>
                    ${speechButtonHtml}
                </div>
        ...
        // Store alert data for speech function
        card.dataset.alertId = alert.id;
        card.dataset.alertText = `${alert.title}. ${localizedContent}`;
        if (Array.isArray(alert.tips) && alert.tips.length) {
            const localizedTips = alert.tips.map(tip => localizeText(tip, alert.location));
            card.dataset.alertTips = localizedTips.join('. ');
        }
        
        return card;
    }

    // Global function for speech buttons
    window.speakAlert = function(alertId) {
        const alertCard = document.querySelector(`[data-alert-id="${alertId}"]`);
        if (alertCard) {
            const alertText = alertCard.dataset.alertText;
            const alertTips = alertCard.dataset.alertTips;
            let fullText = alertText;
            
            if (alertTips) {
                fullText += '. Co masz robić? ' + alertTips;
            }
    ...
    function displayFilteredAlerts() {
        ...
        if (!filtered.length) {
            ...
            // Automatically read the first warning alert aloud
            const firstWarning = filtered.find(alert => alert.severity === 'warning');
            if (firstWarning && speechEnabled) {
                setTimeout(() => {
                    const alertText = `Ważne ostrzeżenie! ${firstWarning.title}. ${localizeText(firstWarning.simplified_content || firstWarning.content, firstWarning.location)}`;
                    speakText(alertText, currentLang);
                }, 2000);
            }
            ...
            if (hasWeatherAlert) {
                setTimeout(() => updateMascotMessage('weatherWarning'), 5000);
            }
        }
    ...
    function updateUILanguage(lang) {
        ...
        const speechBtn = document.getElementById('speech-toggle-btn');
        if (speechBtn && speechBtn.querySelector('.btn-text')) {
            speechBtn.querySelector('.btn-text').textContent = speechEnabled ? 
                (lang === 'pl' ? 'Czytanie włączone' : lang === 'en' ? 'Reading enabled' : 'Читання увімкнено') :
                (lang === 'pl' ? 'Czytanie wyłączone' : lang === 'en' ? 'Reading disabled' : 'Читання вимкнено');
        }
    }
